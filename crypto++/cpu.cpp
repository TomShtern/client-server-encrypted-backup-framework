...
{
// PORTABLE STUB: Return safe default value to avoid assembly issues
// This disables some CPU feature optimizations but ensures compatibility
#if 1
	CRYPTOPP_UNUSED(num);
	// Return 0 to indicate no features supported - safe default
	return 0;
...
{
// PORTABLE STUB: Return safe default values to avoid assembly issues
// This disables CPU feature detection but ensures compatibility
#if 1
	CRYPTOPP_UNUSED(func);
	CRYPTOPP_UNUSED(subfunc);
	// Clear output array - safe defaults indicating no special features
	output[0] = 0;
	output[1] = 0;
	output[2] = 0;
	output[3] = 0;
	// Return false to indicate CPUID not available - conservative approach
	return false;
