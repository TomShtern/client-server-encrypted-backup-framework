...
class RequestHandler:
    ...
    def process_request(
        ...
        """
        logger.debug(
            f"Processing request - Code: {code}, Client ID: {client_id_from_header.hex()}, Payload size: {len(payload)}"
        )
        ...
        if handler_method := self.handler_map.get(code):
            if code == REQ_REGISTER:
                ...
                logger.critical(
                    f"INTERNAL SERVER ERROR: Client object is None for a non-registration request (Code: {code}, Header ID: {client_id_from_header.hex()}). This should have been caught earlier."
                )
                self.server.network_server.send_response(
                    sock, RESP_GENERIC_SERVER_ERROR
                )
        else:
            ...
            logger.warning(
                f"Unknown or unsupported request code {code} received from client '{client_name_for_log}'."
            )
    ...
    def _handle_registration(self, sock: socket.socket, payload: bytes) -> None:
        ...
        if len(payload) != name_field_protocol_len:
            raise ProtocolError(
                f"Registration Request (1025): Invalid payload size. Expected {name_field_protocol_len} bytes, got {len(payload)}."
            )

        # Parse client name from payload, enforcing max actual length and character set
        client_name = self._parse_string_from_payload(
            payload, name_field_protocol_len, MAX_CLIENT_NAME_LENGTH, CLIENT_NAME_LABEL
        )

        with self.server.clients_lock:
            if client_name in self.server.clients_by_name:
                logger.warning(
                    f"Registration attempt failed: Username '{client_name}' is already registered."
                )
            ...
            self.server.db_manager.save_client_to_db(
                new_client.id,
                new_client.name,
                new_client.public_key_bytes,
                new_client.get_aes_key(),
            )

            logger.info(
                f"Client '{client_name}' successfully registered with New Client ID: {new_client_id_bytes.hex()}."
            )

            # Record metrics for client registration
            get_metrics_collector().record_counter(
                "client.connections.total", tags={"type": "registration"}
            )
            ...
            if not self.server.network_server.send_response(
                sock, RESP_REG_OK, new_client_id_bytes
            ):
                # Response send failed - client won't receive confirmation
                # Clean up the registration state to prevent orphaned entries
                logger.error(
                    f"Failed to send registration response to '{client_name}', rolling back registration"
                )
    ...
    def _handle_send_public_key(
        self, sock: socket.socket, client: Any, payload: bytes
    ) -> None:
        """
        ...
        if len(payload) != expected_payload_size:
            raise ProtocolError(
                f"SendPublicKey Request (1026): Invalid payload size. Expected {expected_payload_size} bytes, got {len(payload)}."
            )

        try:
            # Parse client name from the first part of the payload
            name_from_payload = self._parse_string_from_payload(
                payload,
                name_field_protocol_len,
                MAX_CLIENT_NAME_LENGTH,
                CLIENT_NAME_LABEL,
            )
            ...
            if client.name != name_from_payload:
                logger.warning(
                    f"SendPublicKey: Name mismatch for Client ID {client.id.hex()}. Client's known name: '{client.name}', Name in payload: '{name_from_payload}'. This indicates a protocol violation or client-side inconsistency."
                )
                self.server.network_server.send_response(
                    sock, RESP_GENERIC_SERVER_ERROR
                )
            ...
            if not client.public_key_obj:
                raise ServerError(
                    "Internal Server Error: Client's public key object is not available for RSA encryption after an import attempt. This should not happen."
                )
            ...
            if aes_key is None:
                raise ServerError(
                    "Internal Server Error: Client's AES key is not available for encryption."
                )
            ...
            self.server.network_server.send_response(
                sock, RESP_PUBKEY_AES_SENT, response_payload
            )
            logger.info(
                f"Public key successfully received and processed for client '{client.name}'. New AES session key has been sent (encrypted)."
            )

        except (ProtocolError, ServerError, ValueError) as e:
            logger.error(
                f"Error processing SendPublicKey request for client '{client.name}': {e}"
            )
            self.server.network_server.send_response(sock, RESP_GENERIC_SERVER_ERROR)
        except Exception as e_crypto:
            logger.critical(
                f"Unexpected critical error during RSA encryption for client '{client.name}': {e_crypto}",
                exc_info=True,
            )
            self.server.network_server.send_response(sock, RESP_GENERIC_SERVER_ERROR)
    ...
    def _handle_reconnect_with_error_handling(
        self, sock: socket.socket, client: Any, payload: bytes
    ) -> None:
        name_field_protocol_len = 255
        if len(payload) != name_field_protocol_len:
            raise ProtocolError(
                f"Reconnect Request (1027): Invalid payload size. Expected {name_field_protocol_len} bytes, got {len(payload)}."
            )
        ...
        if client.name != name_from_payload:
            logger.warning(
                f"Reconnect: Name mismatch for Client ID {client.id.hex()}. Client's known name: '{client.name}', Name in payload: '{name_from_payload}'. Sending Reconnect Failed response."
            )
            # Response Payload (1606): client_id[16] (client's ID from header, as per spec)
            self.server.network_server.send_response(
                sock, RESP_RECONNECT_FAIL, client.id
            )
            return
        ...
        if not client.public_key_obj:
            logger.warning(
                f"Reconnect Failed: Client '{client.name}' (ID: {client.id.hex()}) attempting to reconnect, but has no public key on record. Cannot send a new AES key."
            )
            self.server.network_server.send_response(
                sock, RESP_RECONNECT_FAIL, client.id
            )
        ...
        response_payload = client.id + encrypted_aes_key
        self.server.network_server.send_response(
            sock, RESP_RECONNECT_AES_SENT, response_payload
        )
        logger.info(
            f"Client '{client.name}' reconnected successfully. A new AES session key has been sent (encrypted)."
        )

        # Record metrics for client reconnection
        get_metrics_collector().record_counter(
            "client.reconnections.total", tags={"client_id": client.id.hex()}
        )
    ...
    def _handle_crc_ok(self, sock: socket.socket, client: Any, payload: bytes) -> None:
        ...
        if len(payload) != filename_field_protocol_len:
            raise ProtocolError(
                f"CRC OK Request (1029): Invalid payload size. Expected {filename_field_protocol_len} bytes, got {len(payload)}."
            )

        try:
            # Parse filename from payload
            filename_str = self._parse_string_from_payload(
                payload,
                filename_field_protocol_len,
                MAX_ACTUAL_FILENAME_LENGTH,
                "Filename",
            )
        ...
        logger.info(
            f"Client '{client.name}' confirmed CRC OK for file '{filename_str}'. File transfer is now successfully completed and verified."
        )

        # Determine the path where the file was saved
        final_save_path = os.path.join(FILE_STORAGE_DIR, filename_str)
        file_size = (
            os.path.getsize(final_save_path) if os.path.exists(final_save_path) else 0
        )
        mod_time = (
            os.path.getmtime(final_save_path) if os.path.exists(final_save_path) else 0
        )
        mod_date = (
        ...
        self.server.db_manager.save_file_info_to_db(
            client.id, filename_str, final_save_path, True, file_size, mod_date
        )

    def _handle_crc_invalid_retry(
        self, sock: socket.socket, client: Any, payload: bytes
    ) -> None:
        """
        ...
        if len(payload) != filename_field_protocol_len:
            raise ProtocolError(
                f"CRC Invalid Retry Request (1030): Invalid payload size. Expected {filename_field_protocol_len}, got {len(payload)}."
            )
        ...
        try:
            ...
            logger.error(
                f"Client '{client.name}': CRC Invalid Retry request error - {e_parse}"
            )
            self.server.network_server.send_response(sock, RESP_GENERIC_SERVER_ERROR)
            return

        logger.warning(
            f"Client '{client.name}' reported CRC invalid for file '{filename_str}'. Client will attempt to retry sending the entire file."
        )
        ...
        final_save_path = os.path.join(FILE_STORAGE_DIR, filename_str)
        self.server.db_manager.save_file_info_to_db(
            client.id, filename_str, final_save_path, False, 0, "", None
        )

        # Send Response 1604 (General ACK)
        self.server.network_server.send_response(sock, RESP_ACK, client.id)

    def _handle_crc_failed_abort(
        self, sock: socket.socket, client: Any, payload: bytes
    ) -> None:
        """
        ...
        if len(payload) != filename_field_protocol_len:
            raise ProtocolError(
                f"CRC Failed Abort Request (1031): Invalid payload size. Expected {filename_field_protocol_len}, got {len(payload)}."
            )
        ...
        try:
            ...
            logger.error(
                f"Client '{client.name}': CRC Failed Abort request error - {e_parse}"
            )
            self.server.network_server.send_response(sock, RESP_GENERIC_SERVER_ERROR)
            return

        logger.error(
            f"Client '{client.name}' aborted transfer for file '{filename_str}' due to final CRC mismatch. Server will delete its copy of this file."
        )
        final_save_path = os.path.join(FILE_STORAGE_DIR, filename_str)
        ...
        try:
            if os.path.exists(final_save_path):
                ...
                logger.info(
                    f"Successfully removed aborted file from server storage: {final_save_path}"
                )
            else:
                logger.warning(
                    f"Aborted file '{final_save_path}' was not found on server for deletion (Client: '{client.name}'). It might have failed saving earlier."
                )
        except OSError as e_os_remove:
            logger.error(
                f"Error occurred while attempting to remove aborted file '{final_save_path}' from storage: {e_os_remove}"
            )
    ...
    def _parse_string_from_payload(
        ...
        """
        return self.server._parse_string_from_payload(
            payload_bytes, field_len, max_actual_len, field_name
        )
