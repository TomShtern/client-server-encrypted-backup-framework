...
class FileTransferManager:
    ...
    def __init__(self, server_instance: Any) -> None:
        ...
        logger.info(
            f"FileTransferManager initialized (version={__FTM_VERSION__}) "
    ...
    def handle_send_file(
        ...
        try:
            ...
            logger.critical(
                f"Unexpected error during file transfer for client '{client.name}': {e}",
                exc_info=True,
            )
            self.server.network_server.send_response(sock, RESP_GENERIC_SERVER_ERROR)
    ...
    def _process_file_transfer_packet(self, payload, client, sock):
        ...
        if not aes_key:
            raise ClientError(
                f"Client '{client.name}' has no active AES key for file decryption"
            )  # type: ignore

        # Log the file transfer progress with enhanced debugging
        logger.info(
            f"Client '{client.name}': Receiving file '{metadata['filename']}', "  # type: ignore
        ...
        if metadata["total_packets"] > 1:
            logger.debug(
                f"Multi-packet transfer: Client '{client.name}', File '{metadata['filename']}', "  # type: ignore
                f"Packet {metadata['packet_number']}/{metadata['total_packets']}, "
        ...
        if metadata["total_packets"] > 1:
            logger.debug(
                f"Reassembly status: Client '{client.name}', File '{metadata['filename']}', "  # type: ignore
                f"Packet {metadata['packet_number']}/{metadata['total_packets']}, "
        ...
        if is_complete:
            ...
            logger.debug(
                f"COMPLETE TRANSFER: Processing complete file '{metadata['filename']}' for client '{client.name}'"
            )  # type: ignore
    ...
    def _parse_file_transfer_metadata(self, payload: bytes) -> dict[str, Any]:
        ...
        if len(payload) < metadata_header_size:
            ...
            raise ProtocolError(
                f"Payload too short for file metadata: {len(payload)} < {metadata_header_size}"
            )
        ...
        self._validate_transfer_metadata(
            encrypted_size, original_size, packet_number, total_packets
        )
        ...
        if len(actual_content) != encrypted_size:
            ...
            raise ProtocolError(
                f"Content size mismatch: declared {encrypted_size}, actual {len(actual_content)}"
            )
    ...
    def _validate_transfer_metadata(
        ...
        if not (0 < encrypted_size <= MAX_PAYLOAD_READ_LIMIT):
            logger.error(
                f"XFER/VALIDATE_ERR: invalid_encrypted_size value={encrypted_size}"
            )
            raise ProtocolError(f"Invalid encrypted_size: {encrypted_size}")

        if not (0 <= original_size <= MAX_ORIGINAL_FILE_SIZE):
            logger.error(
                f"XFER/VALIDATE_ERR: invalid_original_size value={original_size}"
            )
            raise ProtocolError(f"Invalid original_size: {original_size}")

        if total_packets <= 0:
            logger.error(
                f"XFER/VALIDATE_ERR: invalid_total_packets value={total_packets}"
            )
        ...
        if not (1 <= packet_number <= total_packets):
            ...
            raise ProtocolError(
                f"Invalid packet_number {packet_number} for {total_packets} total packets"
            )
    ...
    def _handle_packet_reassembly(self, client: Any, metadata: dict[str, Any]) -> bool:
        ...
        with file_lock:  # Thread-safe access to this specific file transfer
            ...
            if packet_number == 1:
                self._initialize_new_transfer(
                    client, filename, total_packets, metadata["original_size"]
                )
            ...
            self._validate_transfer_consistency(
                client,
                filename,
                file_state,
                total_packets,
            ...
            self._store_packet(
                client, filename, packet_number, metadata["content"], file_state
            )

            # Emit concise state snapshot
            self._log_reassembly_state(client, filename, file_state, total_packets)

            # Check if transfer is complete
            return self._check_transfer_completion(
                client, filename, file_state, total_packets
            )

    def _initialize_new_transfer(
        self, client: Any, filename: str, total_packets: int, original_size: int
    ) -> None:
        """Initializes state for a new file transfer."""
        if filename in client.partial_files:
            logger.warning(
                f"Client '{client.name}': Restarting transfer for '{filename}'"
            )
            # Clean up old transfer from memory manager
            try:
                self._memory_transfer_manager.remove_transfer(
                    client.id, filename, "restart"
                )
            except Exception as e:
        ...
        if not self._memory_transfer_manager.create_transfer(
            client.id, filename, total_packets, original_size
        ):
            raise FileError(
                f"Failed to create transfer - memory limits exceeded: '{filename}'"
            )
    ...
    def _validate_transfer_consistency(
        ...
        """Validates that the current packet matches the ongoing transfer metadata."""
        if not file_state or (
            packet_number > 1
            and (
                file_state["total_packets"] != total_packets
                or file_state["original_size"] != original_size
            )
        ):
            logger.error(
                f"Client '{client.name}': Inconsistent metadata for '{filename}'"
            )
            if file_state:
    ...
    def _store_packet(
        ...
        """Stores a received packet in memory, checking limits."""
        # First check with memory manager
        if not self._memory_transfer_manager.add_packet(
            client.id, filename, packet_number, content
        ):
            logger.warning(
                f"Memory limit exceeded for packet {packet_number} of '{filename}'"
            )
            raise FileError(f"Memory limit exceeded during transfer: '{filename}'")

        # Store in legacy state (for compatibility)
        file_state["received_chunks"][packet_number] = content
        file_state["timestamp"] = time.monotonic()

    def _log_reassembly_state(
        self, client: Any, filename: str, file_state: dict[str, Any], total_packets: int
    ) -> None:
        """Logs the current state of reassembly."""
        try:
            received_count = len(file_state["received_chunks"])
            # For efficiency, only compute cumulative bytes for small N
            cumulative_enc = (
                sum(len(ch) for ch in file_state["received_chunks"].values())
    ...
    def _check_transfer_completion(
        self, client: Any, filename: str, file_state: dict[str, Any], total_packets: int
    ) -> bool:
        """Checks if all packets have been received and validates them."""
        if len(file_state["received_chunks"]) == total_packets:
            ...
            if received_packets != expected_packets:
                ...
                client.clear_partial_file(filename)
                raise ProtocolError(
                    f"File transfer packet number mismatch for '{filename}'"
                )

            return True
        return False

    def _process_complete_file(
        self, sock: socket.socket, client: Any, filename: str, aes_key: bytes
    ) -> None:
        """
        ...
        with file_lock:
            ...
            if not file_state:
                raise ServerError(
                    f"File state missing for complete transfer: {filename}"
                )
            ...
            try:
                ...
                if len(decrypted_data) != file_state["original_size"]:
                    raise FileError(
                        f"Decrypted size mismatch for '{filename}': "
                ...
                crc_value = calculate_crc32(
                    decrypted_data
                )  # Keep existing for small files
                ...
                final_path, mod_date = self._save_file_to_storage(
                ...
                self._send_file_crc_response(
                    sock, client, filename, len(full_encrypted_data), crc_value
                )

                logger.info(
                    f"Client '{client.name}': File '{filename}' processed successfully. "
                ...
                get_metrics_collector().record_counter(
                    "file.uploads.total", tags={"client_id": client.id.hex()}
                )
                ...
                log_memory_efficiency(
                    f"file_upload_{filename}", len(decrypted_data), memory_delta
                )

                # Clean up transfer from memory manager
                self._memory_transfer_manager.remove_transfer(
                    client.id, filename, "completed"
                )
        ...
        try:
            ...
            for i in range(1, total_packets + 1):
                if i not in file_state["received_chunks"]:
                    logger.error(
                        f"XFER/REASM_ERR: missing_packet index={i} of {total_packets}"
                    )
    ...
    def _decrypt_file_data(self, encrypted_data: bytes, aes_key: bytes) -> bytes:
        ...
        try:
            ...
            # Protocol requires Zero IV for file transfer encryption
            # This is a fixed protocol constraint that cannot be changed without breaking clients
            with contextlib.suppress(
                Exception
            ):  # Suppress potential warnings about insecure mode if any
                pass
    ...
    def _save_file_to_storage(self, filename: str, data: bytes) -> tuple[str, str]:
        ...
        temp_path = os.path.join(
            FILE_STORAGE_DIR, f"{filename}.{temp_id}.tmp_EncryptedBackup"
        )
        final_path = os.path.join(FILE_STORAGE_DIR, filename)

        try:
            return self._perform_atomic_file_save(temp_path, data, final_path, filename)
        except Exception as e:
    ...
    def _perform_atomic_file_save(
        self, temp_path: str, data: bytes, final_path: str, filename: str
    ) -> tuple[str, str]:
        """
        ...
        if os.path.exists(final_path):
            ...
            logger.debug(
                f"Removed existing file '{final_path}' before saving new version"
            )
    ...
    def _send_file_crc_response(
        ...
        filename_padded = filename_bytes + b"\0" * (
            MAX_FILENAME_FIELD_SIZE - len(filename_bytes)
        )
        ...
        logger.info(
            f"Client '{client.name}': Sent CRC response for '{filename}' (CRC: {crc_value})"
        )
    ...
    def _update_gui_stats(
        self, filename: str, client_name: str, bytes_transferred: int
    ) -> None:
        """
