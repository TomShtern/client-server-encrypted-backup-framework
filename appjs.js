...
class App {
   ...
   */
  async init() {
    // Make entire initialization non-blocking to allow app to load immediately
    this.connectionMonitor.start();

    // Start socket in background
    this.socket.start().catch(error => {
    ...
    // Return immediately so app loads without waiting
  ...
  async #bootstrap() {
    // Don't wait for initial status - let the app load immediately
  ...
  #validateInput(input, icon) {
    ...
    icon.classList.toggle('success', isValid);
    icon.classList.toggle('error', !isValid);
  ...
  #handleModalKeydown(event) {
    ...
    const last = this.modalFocusables.at(-1);
  ...
  #onSocketDisconnect(reason) {
    ...
    this.logs.add(`Realtime channel disconnected${reasonText}`, { level: 'warn', phase: 'SOCKET' });
  ...
  #handleFileReceipt(payload) {
    ...
    const descriptor = typeof type === 'string' ? type.replaceAll('_', ' ') : 'File event';
    ...
    if (data?.verified || /complete|verified/i.test(descriptor)) {
      ...
      if (globalThis.Enhancements?.showNotification) {
        globalThis.Enhancements.showNotification('Backup Verified', {
  ...
  #deriveUIState(parsed, previousState, speed) {
    const {
      ...
      bytesTransferred: prevTransferred,
      progress: prevProgress,
      elapsedSeconds: prevElapsed,
      startTimestamp,
      connectionLatency: prevLatency,
      connectionQuality: prevQuality,
      jobStatus: prevJobStatus
    } = previousState;

    const {
      totalBytes: parsedTotal,
      bytesTransferred: parsedTransferred,
    ...
    const total = Number.isFinite(parsedTotal) ? parsedTotal : prevTotal;
    const transferred = Number.isFinite(parsedTransferred) ? parsedTransferred : prevTransferred;
    const progress = Number.isFinite(parsedProgress) ? parsedProgress : prevProgress;
    ...
    let elapsedSeconds = prevElapsed;
    if (startTimestamp) {
    ...
    return {
      jobId,
      connected,
      connectionLatency: latency ?? prevLatency,
      connectionQuality,
      jobRunning,
      jobPhase: phase,
      jobMessage: message,
      jobStatus: this.#deriveJobStatus({ phase }, prevJobStatus),
      progress,
  ...
  #handleJobCompletion(jobRunning, jobStatus, jobId) {
    if (!jobRunning && jobStatus === 'completed') {
      ...
      if (globalThis.Enhancements?.showNotification) {
        globalThis.Enhancements.showNotification('Backup Completed', {
          body: `Backup of ${this.state.snapshot.fileName || 'file'} finished successfully.`
        });
      }
    }

    if (!jobRunning && this.state.snapshot.jobId && (!jobId || jobId === this.state.snapshot.jobId)) {
      this.socket.clearJob();
    }
  }
  ...
  #renderConnection(state) {
    ...
    // Update connection status if element exists (removed in new dual-server design)
    this.#updateClass(dom.connStatus, badgeClass);
    this.#updateText(dom.connStatus, statusText);

    const healthText = `Latency ${formatLatency(state.connectionLatency)}`;
    this.#updateText(dom.connHealth, healthText);
    ...
    // Update connection quality if element exists (removed in new dual-server design)
    this.#updateClass(dom.connQuality, qualityClass);
    this.#updateText(dom.connQuality, qualityLabel);
  }
  ...
  #renderProgress(state) {
    ...
    if (dom.phaseText.textContent !== phaseLabel) {
      ...
      this.#updateText(dom.phaseText, phaseLabel);
    }
    ...
    if (!currentOffset || Math.abs(Number.parseFloat(currentOffset) - offset) > 0.5) {
    ...
    if (dom.progressPct.textContent !== progressText) {
      ...
      this.#updateText(dom.progressPct, progressText);
    }

    const etaText = etaSeconds ? formatDuration(etaSeconds) : '–';
    this.#updateText(dom.etaText, etaText);
  }
  ...
  #renderStats(state) {
    ...
    for (const card of this.#cachedStatCards) {
      if (isTransferring) {
    ...
    if (dom.stats.bytes.textContent !== bytesText) {
      ...
      this.#updateText(dom.stats.bytes, bytesText);
    }

    // Throttle speed updates to max 10/sec (reduce animation overhead)
    const now = performance.now();
    const speedText = formatSpeed(state.speed);
    if (now - this.#lastSpeedUpdate >= this.#speedUpdateThrottle && dom.stats.speed.textContent !== speedText) {
      animateOnce(dom.stats.speed, 'updating');
      this.#updateText(dom.stats.speed, speedText);
    ...
    if (dom.stats.size.textContent !== sizeText) {
      ...
      this.#updateText(dom.stats.size, sizeText);
    }
    ...
    if (dom.stats.elapsed.textContent !== elapsedText) {
      ...
      this.#updateText(dom.stats.elapsed, elapsedText);
    }

    // Update speed chart if available
    if (globalThis.Enhancements && typeof globalThis.Enhancements.updateSpeedChart === 'function') {
      globalThis.Enhancements.updateSpeedChart(state.speed);
    }
  }
